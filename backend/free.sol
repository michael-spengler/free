...
contract Freedom is ERC20, Ownable {
    ...
    uint256 public numberOfFreedomFans = 0; // this number will hopefully increase over time
    uint256 public numberOfApprovedFreedomFans = 0; // this number will hopefully also increase over time
    uint256 public currentThresholdForBecomingFullyApproved = 1; // this threshold increases proportionally to the number of approved FreedomFans
    ...
    mapping(uint256 => address) invitedMembersOfTrustedInitiationCommunity; // in this mapping we store who shall be invited and approved already during the start of this project
    ...
    constructor() ERC20("Freedom", "FREE") {
        ...
        invitedMembersOfTrustedInitiationCommunity[1] = msg.sender; // e.g. the developer - you can add more trusted intitiators here
    }
    ...
    function approveFreedomFan(address freedomFan) public avoidDuplicateApprovals(freedomFan) onlyNotYetApprovedFreedomFansCanBeApproved(freedomFan) {
        require(freedomFans[ids[msg.sender]].approvedOn > 0 || owner() == msg.sender, "only approved freedom fans or the owner at the beginning can do that.");
    ...
    // the following funciton can only be executed once by the owner. at the end of startProject this contract is automatically renounced. 
    function startProject() public onlyOwner { 
        for (uint16 i = 1; i <= 24; i++){ // assuming there are a maximum of 24 invited members of the trusted initiation community 
            if (invitedMembersOfTrustedInitiationCommunity[i] == 0x0000000000000000000000000000000000000000) {
                return;
            }
            updateNumberOfFreedomFansAndIDs(invitedMembersOfTrustedInitiationCommunity[i]);
            updateCoreDataForFreedomFan(invitedMembersOfTrustedInitiationCommunity[i], "https://twitter.com/Peer2peerE/status/1695323724646322412");
            approveFreedomFan(invitedMembersOfTrustedInitiationCommunity[i]);
        }
        renounceOwnership();
    }
